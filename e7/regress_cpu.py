...
def get_data(filename):
    ...
    # should be the temperature value from the next row
    sysinfo[y_column] = sysinfo['temperature'].shift(periods=-1)

    # the last row should have y_column null: no next temp known
...
def output_regression(coefficients):
    regress = ' + '.join(
        '%.3g*%s' % (coef, col) for col, coef in zip(X_columns, coefficients)
    )
