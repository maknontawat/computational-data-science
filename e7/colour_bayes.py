...
name_to_rgb = np.vectorize(
    COLOUR_RGB.get,
    otypes=[np.uint8, np.uint8, np.uint8])
...
def print_accuracy(model, test_data):
    X_test, y_test = test_data
    print("Testing score: %g" % model.score(X_test, y_test))
    return
...
def main(infile):
    ...
    X = data[X_columns] / 255
    y = data[y_column]  # array with shape (n,) of colour words.
    ...
    print_accuracy(model_rgb, (X_test, y_test))
    ...
    print_accuracy(model_lab, (X_test, y_test))
