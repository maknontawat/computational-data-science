...
class TestSorts(unittest.TestCase):
    ...
    def _test_impl(self, sort, arr, orig_arr):
        ...
        result = sort(arr)
        self.assertTrue(np.array_equal(arr, orig_arr),
                        '%s modified input' % (sort))
        self.assertTrue(self._is_sorted(
            result), "%s didn't correctly sort\n%s\n%s" % (sort, arr, result))
        self.assertEqual(input_counts, Counter(result),
                         "%s didn't preserve the elements" % (sort))
        self.assertIsInstance(result, np.ndarray)
        self.assertEqual(result.shape, orig_arr.shape)
        self.assertEqual(result.dtype, orig_arr.dtype,
                         "%s changed types" % (sort))
    ...
    def test_implementations(self):
        n = 100
        for i in range(5):
            self._test_arr(np.random.randint(
                10000, 100 * n + 10000, size=(n,)))
