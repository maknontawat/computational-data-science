...
def pivot_months_pandas(data):
    ...
    df['month'] = pd.Series(df['date']).apply(lambda x: date_to_month(x))

    # Aggregate over the name and month columns.
    grouped = df['precipitation'].groupby([df['name'], df['month']])

    # Sum each of the aggregated values to get the total.
    grouped_sum_df = pd.DataFrame(grouped.aggregate('sum').reset_index())
    monthly = grouped_sum_df.pivot(
        index='name', columns='month', values='precipitation')

    # Count each of the aggregated values to get the count.
    grouped_count_df = pd.DataFrame(grouped.aggregate('count').reset_index())
    counts = grouped_count_df.pivot(
        index='name', columns='month', values='precipitation')
